...
const DB_CONN = process.env.POSTGRES_CONNECTION_STRING || 'postgresql://mixtape_user:yourpassword@localhost:5432/mixtape';
...
async function main() {
  try {
    ...
    assert(typeof userId === 'number' && userId > 0, '[ASSERT] User ID should be a positive number');
    ...
    assert(origSize === streamedSize, `[ASSERT] Streamed file size (${streamedSize}) should match original (${origSize})`);
    ...
    assert(deleteRes.data && deleteRes.data.message && deleteRes.data.message.includes('deleted'), '[ASSERT] Delete response should confirm deletion');
    // File should be deleted
    const uploadedFilename = track.file_url.split('/').pop();
    const uploadedFilePath = path.resolve(__dirname, '../../_server-data/uploads', uploadedFilename!);
    ...
    try {
      ...
      if (err.response && err.response.status === 404) stream404 = true;
    }
    ...
    try {
      ...
      if (err.response && err.response.status === 404) download404 = true;
    }
